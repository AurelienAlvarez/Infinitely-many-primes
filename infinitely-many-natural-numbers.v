...
Lemma le_sum : forall l : list nat, forall x : nat, In x l -> x <= sum l.
Proof.
  intros l x Hin. induction l ; destruct Hin ; simpl.
  rewrite H. omega. assert (foo := IHl H). omega.
Qed.
...
Lemma le_max : forall l : list nat, forall x : nat, In x l -> x <= maximum l.
Proof.
  intros l x Hin. induction l ; destruct Hin.
  rewrite <- H ; simpl ; apply Max.le_max_l. simpl.
  assert (trans1 := IHl H). assert (trans2 := Max.le_max_r a (maximum l)). omega.
Qed.
...
Proof.
  intro l. induction l.
    ...
    destruct a.
      simpl ; exact IHl.
      simpl. apply lt_le_trans with (n := 0) (m := product l) (p := product l + a * product l).
...
Corollary product_le : forall l : list nat, forall x : nat, max x (product l) <= product (x::l).
Proof.
  intros l x. apply Nat.max_lub ; [apply product_le_x | apply product_le_l].
Qed.

Lemma le_prod : forall l : list nat, forall x : nat, In x l -> x <= product l.
Proof.
  intros l x Hin. induction l. elim Hin. destruct x.
  exact (lt_le_weak 0 (product (a::l)) (product_lt_0 (a::l))). destruct Hin.
  rewrite H. apply Nat.max_lub_l with (n := S x) (m := product l) (p := product (S x::l)) ; exact (product_le l (S x)).
  apply le_trans with (n := S x) (m := product l) (p := product (a::l)). exact (IHl H).
  apply Nat.max_lub_r with (n := a) (m := product l) (p := product (a::l)) ; exact (product_le l (a)).
Qed.
...
Proof.
  ...
  (* assert (absurd := le_sum l N H). *)
  (* assert (absurd := le_prod l N H). *)
  assert (absurd := le_max l N H).
